...
const RARITY_INFO = {
  ...
  legendary: { label: 'Легендарный', color: '#ffb84d', dupValue: 700 },
  mythic:    { label: 'Мифический', color: '#ff4d7a', dupValue: 1500 },
};
...
const CHAR_DB = [
  ...
  { id: 'kopeyshik',     name: 'Копейщик',       rarity: 'common',    attackType: 'melee',  color: '#b7c2cf', baseHp: 110, baseDamage: 12, speed: 150, range: 90,  desc: 'Длинное древко держит врагов на расстоянии.' },
  // rare
  ...
  { id: 'piromant',      name: 'Пиромант',       rarity: 'rare',      attackType: 'ranged', color: '#ff9a4d', baseHp: 100, baseDamage: 15, speed: 145, range: 230, desc: 'Швыряет огненные сгустки, поджигая землю.' },
  // epic
  ...
  { id: 'inkvizitor',    name: 'Инквизитор',     rarity: 'epic',      attackType: 'melee',  color: '#d1667a', baseHp: 130, baseDamage: 24, speed: 165, range: 82,  desc: 'Карает нечисть тяжёлым молотом.' },
  // legendary
  { id: 'phoenix',       name: 'Феникс',         rarity: 'legendary', attackType: 'ranged', color: '#ffb84d', baseHp: 135, baseDamage: 32, speed: 180, range: 340, desc: 'Огненные перья прожигают целые ряды врагов.' },
  { id: 'storm_lord',    name: 'Владыка Бурь',   rarity: 'legendary', attackType: 'melee',  color: '#7ec8ff', baseHp: 150, baseDamage: 34, speed: 190, range: 92,  desc: 'Каждый удар высекает молнию.' },
  // mythic
  { id: 'abyss_avatar',  name: 'Аватар Бездны',  rarity: 'mythic',    attackType: 'melee',  color: '#ff4d7a', baseHp: 180, baseDamage: 44, speed: 200, range: 100, desc: 'Существо из другого мира, крушит всё на своём пути.' },
  { id: 'dawn_titan',    name: 'Титан Рассвета', rarity: 'mythic',    attackType: 'ranged', color: '#ffe066', baseHp: 170, baseDamage: 40, speed: 170, range: 400, desc: 'Свет рассвета выжигает орды врагов издалека.' },
];

const BOX_DB = [
  { id: 'field',   name: 'Полевой ящик',    icon: '📦', cost: 120,  odds: { common: 0.66, rare: 0.27, epic: 0.06, legendary: 0.009, mythic: 0.001 } },
  { id: 'assault', name: 'Штурмовой ящик',  icon: '🗝️', cost: 450,  odds: { common: 0.28, rare: 0.48, epic: 0.19, legendary: 0.045, mythic: 0.005 } },
  { id: 'mythic',  name: 'Мифический ящик', icon: '🔮', cost: 1400, odds: { rare: 0.30, epic: 0.40, legendary: 0.22, mythic: 0.08 } },
];
...
const AVATAR_ICONS = ['🙂', '🦊', '🐺', '🦉', '💀', '🔥', '🌙', '⚡', '🐍', '🌲'];
const THEMES = [
  { id: 'steel',  color: '#7b9bc4', label: 'Сталь' },
  { id: 'slate',  color: '#8f99ab', label: 'Графит' },
  { id: 'ice',    color: '#8fd3ff', label: 'Лёд' },
  { id: 'indigo', color: '#7285d9', label: 'Индиго' },
  { id: 'storm',  color: '#5c7ea6', label: 'Шторм' },
];
...
if (save.potions === undefined) save.potions = 0;
...
function switchUser(id) {
  ...
  applyTheme(currentUser.theme || DEFAULT_THEME);
  renderAll();
}
...
function setCloudError(text, elId) {
  const el = document.getElementById(elId || 'cloud-error');
...
async function cloudRegister(username, password, errElId) {
  if (!cloudReady) return setCloudError('Облако ещё не настроено — см. инструкцию.', errElId);
  if (!validUsername(username)) return setCloudError('Юзернейм: 3–16 символов, латиница/цифры/подчёркивание.', errElId);
  if (password.length < 6) return setCloudError('Пароль минимум 6 символов.', errElId);
  setCloudError('Регистрируем…', errElId);
  try {
    const cred = await fbAuth.createUserWithEmailAndPassword(usernameToEmail(username), password);
    const doc = {
      username, avatarColor: pick(AVATAR_COLORS), avatarIcon: pick(AVATAR_ICONS), theme: DEFAULT_THEME,
      ...defaultUserSave(), updatedAt: Date.now(),
    };
    await fbDb.collection('users').doc(cred.user.uid).set(doc);
    setCloudError('', errElId);
  } catch (e) {
    setCloudError(cloudErrorText(e), errElId);
  }
}

async function cloudLogin(username, password, errElId) {
  if (!cloudReady) return setCloudError('Облако ещё не настроено — см. инструкцию.', errElId);
  setCloudError('Входим…', errElId);
  try {
...
async function onCloudAuthChanged(user) {
  const wasOnAuthScreen = document.getElementById('screen-auth').classList.contains('active');
  if (user) {
    cloudUid = user.uid;
    let data;
    try {
    ...
    if (!data) {
      data = { username: user.email.split('@')[0], avatarColor: pick(AVATAR_COLORS), avatarIcon: pick(AVATAR_ICONS), theme: DEFAULT_THEME, ...defaultUserSave() };
      await fbDb.collection('users').doc(user.uid).set(data);
    }
    localBackup = { currentUser, save };
    currentUser = {
      id: 'cloud_' + user.uid, isCloud: true,
      nickname: data.username, avatarColor: data.avatarColor, avatarIcon: data.avatarIcon, theme: data.theme || DEFAULT_THEME,
    };
    save = {
    ...
    if (wasOnAuthScreen) { showScreen('screen-menu'); return; }
  } else {
    cloudUid = null;
    if (localBackup) {
...
document.getElementById('cloud-register-btn').addEventListener('click', () => {
  cloudRegister(document.getElementById('cloud-username').value, document.getElementById('cloud-password').value, 'cloud-error');
});
document.getElementById('cloud-login-btn').addEventListener('click', () => {
  cloudLogin(document.getElementById('cloud-username').value, document.getElementById('cloud-password').value, 'cloud-error');
});
...
document.getElementById('auth-register-btn').addEventListener('click', () => {
  cloudRegister(document.getElementById('auth-username').value, document.getElementById('auth-password').value, 'auth-error');
});
document.getElementById('auth-login-btn').addEventListener('click', () => {
  cloudLogin(document.getElementById('auth-username').value, document.getElementById('auth-password').value, 'auth-error');
});

/* =========================================================================
   nickname modal (replaces window.prompt, which is unreliable in some
   mobile / embedded webviews and was breaking "create new profile")
   ========================================================================= */

function openNickModal(defaultVal, onConfirm) {
...
function showScreen(id) {
  ...
  if (id === 'screen-auth') renderAuthScreen();
...
function renderAll() {
  ...
  if (active && active.id === 'screen-auth') renderAuthScreen();
...
function renderAuthScreen() {
  const qa = document.getElementById('auth-quick-avatar');
  qa.textContent = currentUser.avatarIcon;
  qa.style.background = currentUser.avatarColor;
  document.getElementById('auth-quick-name').textContent = currentUser.nickname;
  document.getElementById('auth-quick-continue').classList.toggle('hidden', !!currentUser.isCloud);

  const list = document.getElementById('auth-profiles-list');
  list.innerHTML = '';
  loadUsers().filter((u) => u.id !== currentUser.id).forEach((u) => {
    const row = document.createElement('div');
    row.className = 'auth-quick-row';
    row.innerHTML = `
      ...
      <button class="btn btn-small" data-quick-switch="${u.id}">Продолжить</button>
    `;
    list.appendChild(row);
  });
  list.querySelectorAll('[data-quick-switch]').forEach((b) => {
    b.addEventListener('click', () => { switchUser(b.dataset.quickSwitch); showScreen('screen-menu'); });
  });
}

document.getElementById('auth-quick-btn').addEventListener('click', () => showScreen('screen-menu'));
document.getElementById('auth-new-guest-btn').addEventListener('click', () => {
  openNickModal('Игрок' + (loadUsers().length + 1), (nick) => {
    const u = createUser(nick);
    switchUser(u.id);
...
function renderProfile() {
  ...
  THEMES.forEach((t) => {
    const sw = document.createElement('div');
    sw.className = 'swatch' + ((currentUser.theme || DEFAULT_THEME) === t.id ? ' selected' : '');
...
function renderCharacters() {
  ...
  CHAR_DB.forEach((def) => {
    ...
    card.className = 'char-card' + (!st.owned ? ' locked' : '') + (save.selected === def.id ? ' selected' : '') +
      (def.rarity === 'legendary' ? ' legendary' : '') + (def.rarity === 'mythic' ? ' mythic' : '');
    card.style.setProperty('--rc', rarity.color);

    if (!st.owned) {
      card.innerHTML = `
        <div class="char-top">
          <div class="char-avatar" style="background:#2a2e35;"></div>
...
function renderBoxShop() {
  ...
  BOX_DB.forEach((box) => {
    ...
    el.className = 'box-card' + (box.id === 'mythic' ? ' mythic-box' : '');
    const oddsHtml = Object.keys(box.odds)
      .filter((r) => box.odds[r] > 0)
      .map((r) => `${RARITY_INFO[r].label} <span>${fmtPct(box.odds[r])}</span>`)
      .join(' · ');
    el.innerHTML = `
      <div class="box-icon">${box.icon}</div>
      <div class="box-name">${box.name}</div>
      <div class="box-odds">${oddsHtml}</div>
...
function renderPotionShop() {
  const row = document.getElementById('potion-row');
  row.innerHTML = `
    <div class="potion-card">
      <div class="potion-icon">🧪</div>
      <div class="potion-info">
        <div class="potion-name">Зелье лечения</div>
        <div class="potion-sub">Восстанавливает ${Math.round(POTION_HEAL_PCT * 100)}% макс. здоровья во время вылазки — клавиша R или значок в бою.</div>
        <div class="potion-owned">В запасе: ${save.potions || 0}</div>
      </div>
      <button class="btn btn-primary" id="buy-potion-btn" ${save.coins < POTION_COST ? 'disabled' : ''}>+1 · ${POTION_COST}</button>
    </div>
  `;
  document.getElementById('buy-potion-btn').addEventListener('click', () => {
    if (save.coins < POTION_COST) return;
    save.coins -= POTION_COST;
    save.potions = (save.potions || 0) + 1;
    persist();
    renderShop();
  });
}
...
function showReveal(charDef, rarity, resultText) {
  ...
  card.className = 'reveal-card' + (rarity === 'legendary' ? ' legendary' : '') + (rarity === 'mythic' ? ' mythic' : '');
  content.innerHTML = `
  ...
  overlay.querySelectorAll('.confetti-piece').forEach((n) => n.remove());
  if (rarity === 'legendary' || rarity === 'mythic') {
    const colors = rarity === 'mythic' ? ['#ff4d7a', '#c58aff', '#6fa8ff', '#ffe066'] : ['#ffb84d', '#ffe066', '#fff2c9'];
    for (let i = 0; i < 36; i++) {
...
// physical-key mapping so WASD/E/Q/R work regardless of OS keyboard layout
// (e.g. Cyrillic ЙЦУКЕН layouts, where e.key for the physical E key isn't "e")
const CODE_TO_ACTION = {
  KeyW: 'w', KeyA: 'a', KeyS: 's', KeyD: 'd', KeyE: 'e', KeyQ: 'q', KeyR: 'r',
  ArrowUp: 'arrowup', ArrowDown: 'arrowdown', ArrowLeft: 'arrowleft', ArrowRight: 'arrowright',
};
function normalizeKey(e) {
  if (e.code && CODE_TO_ACTION[e.code]) return CODE_TO_ACTION[e.code];
...
document.getElementById('btn-potion-touch').addEventListener('click', () => usePotion());

/* ---------- touch: movement joystick + big attack button + action buttons ---------- */
/* Touch AND Pointer events are both wired (pointer events act as a fallback
   for devices/webviews where raw touch events are unreliable — the
   pointerType==='touch' guard stops a real touch from double-firing). */
...
function setupJoystick(baseEl, knobEl, onChange) {
  ...
  function apply(clientX, clientY) {
    ...
    onChange(Math.min(d / maxR, 1), angle, true);
  }
  ...
  baseEl.addEventListener('touchstart', (e) => { e.preventDefault(); const t = e.changedTouches[0]; startAt(t.clientX, t.clientY); }, { passive: false });
  baseEl.addEventListener('touchmove', (e) => { if (!active) return; e.preventDefault(); const t = e.changedTouches[0]; apply(t.clientX, t.clientY); }, { passive: false });
  baseEl.addEventListener('touchend', endAll, { passive: false });
  baseEl.addEventListener('touchcancel', endAll, { passive: false });

  baseEl.addEventListener('pointerdown', (e) => {
    if (e.pointerType === 'touch') return;
    pointerId = e.pointerId;
    try { baseEl.setPointerCapture(pointerId); } catch (err) { /* ignore */ }
    startAt(e.clientX, e.clientY);
  });
  baseEl.addEventListener('pointermove', (e) => { if (pointerId === null || e.pointerId !== pointerId) return; apply(e.clientX, e.clientY); });
  baseEl.addEventListener('pointerup', (e) => { if (e.pointerId !== pointerId) return; endAll(); });
  baseEl.addEventListener('pointercancel', endAll);
}
...
attackBtn.addEventListener('pointerdown', (e) => { if (e.pointerType === 'touch') return; mouse.down = true; });
attackBtn.addEventListener('pointerup', (e) => { if (e.pointerType === 'touch') return; mouse.down = false; });
attackBtn.addEventListener('pointercancel', () => { mouse.down = false; });

function bindTapButton(el, key) {
  el.addEventListener('touchstart', (e) => { e.preventDefault(); justPressed[key] = true; }, { passive: false });
  el.addEventListener('pointerdown', (e) => { if (e.pointerType === 'touch') return; justPressed[key] = true; });
}
bindTapButton(document.getElementById('btn-build-touch'), 'q');

// collect is HOLD-to-gather, not a single tap — much more forgiving on touch
let touchCollectHeld = false;
const collectBtn = document.getElementById('btn-collect-touch');
collectBtn.addEventListener('touchstart', (e) => { e.preventDefault(); touchCollectHeld = true; }, { passive: false });
collectBtn.addEventListener('touchend', (e) => { e.preventDefault(); touchCollectHeld = false; }, { passive: false });
collectBtn.addEventListener('touchcancel', () => { touchCollectHeld = false; });
collectBtn.addEventListener('pointerdown', (e) => { if (e.pointerType === 'touch') return; touchCollectHeld = true; });
collectBtn.addEventListener('pointerup', (e) => { if (e.pointerType === 'touch') return; touchCollectHeld = false; });
collectBtn.addEventListener('pointercancel', () => { touchCollectHeld = false; });
collectBtn.addEventListener('pointerleave', () => { touchCollectHeld = false; });
...
function popElement(id) {
  ...
  if (!el) return;
  el.classList.remove('coin-pop-anim');
  void el.offsetWidth;
  el.classList.add('coin-pop-anim');
}
...
function spawnWave() {
  ...
  if (isBossWave(n)) {
    ...
    gs.enemies.push({
      ...
      damage: 9 + n * 1.4, speed: 62, cooldown: 0, boss: true, hitFlash: 0,
    });
...
function spawnMinion(n) {
  ...
  gs.enemies.push({
    ...
    damage: 5 + n * 1.2, speed: Math.min(60 + n * 4, 150), cooldown: 0, hitFlash: 0,
  });
}
...
function addCoins(n) {
  ...
  persist();
  popElement('hud-run-coins');
}

function usePotion() {
  if (!gs || !gs.running) return;
  if (!save.potions || save.potions <= 0) return;
  const p = gs.player;
  if (p.hp >= p.maxHp) return;
  save.potions -= 1;
  persist();
  p.hp = Math.min(p.maxHp, p.hp + Math.round(p.maxHp * POTION_HEAL_PCT));
  addParticle(p.x, p.y - 30, '+HP (зелье)', '#6be0b0');
  const el = document.getElementById('hud-potions');
  if (el) el.textContent = save.potions;
  popElement('hud-potions');
}
...
function update(dt) {
  ...
  // resource collection — hold E (or the mobile collect button) near a node.
  // Continuous hold-to-gather (rather than a single precisely-timed tap)
  // makes this reliable across keyboards/layouts and touch devices alike.
  if (gs.gatherCooldown > 0) gs.gatherCooldown -= dt;
  const collecting = keys['e'] || touchCollectHeld;
  if (collecting && gs.gatherCooldown <= 0) {
    let nearest = null, nearestD = Infinity;
    for (const node of gs.nodes) {
      const d = dist(p.x, p.y, node.x, node.y);
      if (d < node.r + 46 && d < nearestD) { nearestD = d; nearest = node; }
    }
    if (nearest) {
      nearest.amount -= 1;
      if (nearest.type === 'wood') gs.wood += 1; else gs.stone += 1;
      addParticle(nearest.x, nearest.y - 16, nearest.type === 'wood' ? '+1 дерево' : '+1 камень', '#e7ebf0');
      gs.gatherCooldown = 0.32;
      gs.nodes = gs.nodes.filter((n) => n.amount > 0);
    }
  }

  // heal potion
  if (justPressed['r']) usePotion();
...
function render() {
  ...
  for (const n of gs.nodes) {
    const inRange = dist(gs.player.x, gs.player.y, n.x, n.y) < n.r + 46;
    if (inRange) {
      ctx.beginPath();
      ctx.strokeStyle = 'rgba(123,155,196,0.5)';
      ctx.lineWidth = 2;
  ...
  for (const e of gs.enemies) {
    ctx.beginPath();
    ctx.fillStyle = e.hitFlash > 0 ? '#ffffff' : (e.boss ? '#caa23c' : '#7a3b3b');
    ...
    ctx.fillStyle = e.boss ? '#ffb84d' : '#d9534f';
  ...
  if (buffed) {
    ctx.beginPath();
    ctx.strokeStyle = 'rgba(231,235,240,0.5)';
    ctx.lineWidth = 2;
  ...
  ctx.beginPath();
  ctx.strokeStyle = 'rgba(231,235,240,0.07)';
...
function updateHudStatic() {
  ...
  document.getElementById('hud-potions').textContent = save.potions || 0;
}
function updateHud() {
